...
exports.recommendsPlace = async (req, res) => {
	...
	try {
		const { data } = await axios.post(
			`${process.env.FLASK_API_URL}/recommends`,
			givenData
		);
		const nearByPlaceName = data.userData.near_you.map((item) =>
			item.place_name.toLowerCase()
		);
		const recommendedPlaceName = data.userData.special_recommendation.map(
